...
public interface ApproveQuestionMapper {
    ...
    // 插入审核记录
    @Insert("INSERT INTO approve_question (uploadId, comment, status) VALUES (#{uploadId}, #{comment}, #{status})")
    ...
    // 更新审核状态
    @Update("UPDATE approve_question SET comment = #{comment}, status = #{status} WHERE id = #{id}")
